...
export default function EquipamentoList({ equipamentos, id, className }: Props) {
  return (
    <div id={id} className={className ?? "grid grid-cols-2 gap-5 sm:grid-cols-4"}>
      {equipamentos.map((equipamento) => (
        <CardEquipamento
          key={equipamento.slug}
          equipamento={equipamento}
          categoriaLabel={
            categoriaLabelPorId.get(equipamento.categoria) ?? equipamento.categoria
          }
        />
      ))}
    </div>
